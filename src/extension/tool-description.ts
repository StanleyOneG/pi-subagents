...
• { action: "watchdog.configure", model: "recommended" | "inherit" | "provider/model[:thinking]", scope?: "session" | "user" | "project", target?: "main" | "children" | "child", agent?: "name", thinking?: "inherit" | "off" | "minimal" | "low" | "medium" | "high" | "xhigh" | "max" } - configure watchdog model selection; max requires resolved Pi model metadata thinkingLevelMap.max; default scope is session, use persistent scopes only when the user asks
• { action: "create", config: { name: "custom-agent", package: "code-analysis", systemPrompt, systemPromptMode, inheritProjectContext, inheritSkills, defaultContext, acceptance, acceptanceRole: "read-only" | "writer", requiredSkills, optionalSkills, requiredTools, acceptanceCapability: "read-only" | "mutating", ... } }
• acceptanceRole affects inferred acceptance only, never tool access. For acceptanceRole inference, explicit task mutation/no-edit intent wins; omission preserves name heuristics. acceptanceCapability is trusted role-card metadata used for resource/acceptance enforcement and must agree with acceptanceRole. Required resources fail preflight; optional and legacy skills warn. Update an individual field with false or an empty string to clear it.
